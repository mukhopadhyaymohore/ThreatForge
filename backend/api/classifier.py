...
INCIDENT_PATTERNS = {
    'ransomware': [
        'ransomware', 'encrypted files', 'ransom', 'bitcoin payment',
        'locked files', 'decrypt', 'crypto locker', 'wannacry', 'lockbit'
    ],
    'phishing': [
        'phishing', 'suspicious email', 'credential harvesting',
        'fake login', 'spear phishing', 'whaling', 'email fraud',
        'malicious link', 'spoofed email'
    ],
    'ddos': [
        'ddos', 'denial of service', 'traffic flood', 'server overload',
        'bandwidth exhaustion', 'service unavailable', 'botnet attack'
    ],
    'data_breach': [
        'data breach', 'data leak', 'exfiltration', 'stolen data',
        'pii exposed', 'database dump', 'credentials leaked',
        'sensitive data', 'unauthorized access to records'
    ],
    'insider_threat': [
        'insider', 'malicious employee', 'privilege abuse',
        'unauthorized download', 'data theft by employee',
        'rogue admin', 'disgruntled employee'
    ],
    'malware': [
        'malware', 'trojan', 'worm', 'spyware', 'rootkit',
        'keylogger', 'backdoor', 'virus', 'infected machine'
    ],
    'zero_day': [
        'zero day', '0day', 'unknown vulnerability', 'unpatched exploit',
        'novel attack', 'undisclosed vulnerability'
    ],
    'social_engineering': [
        'social engineering', 'vishing', 'pretexting', 'baiting',
        'impersonation', 'phone scam', 'identity fraud'
    ],
}

SEVERITY_SIGNALS = {
    'critical': ['production down', 'all systems', 'complete outage', 'payment systems',
                 'hospital', 'critical infrastructure', 'nation state', 'entire network'],
    'high':     ['multiple systems', 'customer data', 'financial data', 'spreading fast',
                 'executive', 'admin credentials', 'database'],
    'medium':   ['single system', 'isolated', 'contained', 'one department', 'endpoint'],
...
ORG_SIZE_PATTERNS = {
    ...
    'mid':        ['mid-size', 'medium', 'hundreds of employees', 'regional'],
    'small':      ['small business', 'startup', 'smb', 'few employees', 'small team'],
}
...
def classify_incident(text: str) -> dict:
    ...
    entities = {
        ...
        'systems':       [e.text for e in doc.ents if e.label_ in ('PRODUCT', 'WORK_OF_ART')],
    }
